...
const Messaging = () => {
  ...
  useEffect(() => {
    ...
    const unsubSent = onSnapshot(
      ...
      err => {
        console.error('[Postfach] Fehler beim Laden gesendeter Nachrichten:', err.code, err.message);
        sentReady = true;
        sentMsgs = [];
        if (err.code === 'permission-denied') {
          setError('Keine Berechtigung. Bitte Seite neu laden oder erneut anmelden.');
        } else {
          setError('Verbindung zu Firestore fehlgeschlagen: ' + err.message);
        }
        setLoading(false);
      }
    );
    ...
    const unsubReceived = onSnapshot(
      ...
      err => {
        console.error('[Postfach] Fehler beim Laden empfangener Nachrichten:', err.code, err.message);
        receivedReady = true;
        receivedMsgs = [];
        if (err.code === 'permission-denied') {
          setError('Keine Berechtigung. Bitte Seite neu laden oder erneut anmelden.');
