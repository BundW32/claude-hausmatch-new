...
type LoadMode = 'loading' | 'firestore' | 'api' | 'error_setup' | 'error_other';
...
const AttachmentPreview = ({ att, isMe }: { att: MessageAttachment; isMe: boolean }) => {
  ...
  return (
    <a href={att.url} target="_blank" rel="noopener noreferrer"
      className={`flex items-center gap-3 mt-2 rounded-xl px-3 py-2 max-w-[260px] transition-colors ${isMe ? 'bg-white/15 hover:bg-white/25' : 'bg-slate-100 hover:bg-slate-200'}`}>
...
const Messaging = () => {
  ...
  const buildConversations = useCallback((sent: Message[], received: Message[]): Conversation[] => {
    ...
    return Object.entries(groups)
      .map(([partnerId, msgs]) => {
        ...
        const partnerName = last.senderId === user.id ? (last.receiverName || 'Unbekannt') : (last.senderName || 'Unbekannt');
  ...
  // --- API-based fetch (fallback when Firestore rules block reads) ---
  const fetchViaApi = useCallback(async (showLoading = false): Promise<boolean> => {
    if (!auth.currentUser) return false;
    try {
      const token = await auth.currentUser.getIdToken();
      const res = await fetch('/api/messages', { headers: { Authorization: `Bearer ${token}` } });
      if (res.status === 503 || res.status === 404) return false; // API not available/configured
  ...
  // --- Mark read via API ---
  const markReadViaApi = useCallback(async (messageId: string) => {
  ...
  useEffect(() => {
    ...
    let firestorePermissionDenied = 0;

    const tryUpdateFirestore = () => {
      if (!sentReady || !receivedReady || canceled) return;
      setConversations(buildConversations(sentMsgs, receivedMsgs));
      setMode('firestore');
    };

    const handleFirestoreError = async (err: any) => {
      if (canceled) return;
      if (err.code === 'permission-denied') {
        firestorePermissionDenied++;
        if (firestorePermissionDenied >= 1) {
          // Firestore rules block reads → try API fallback
          unsubSent?.(); unsubSent = null;
          unsubReceived?.(); unsubReceived = null;
          const ok = await fetchViaApi();
          if (!ok) {
            setMode('error_setup');
            setErrorDetail('');
          } else {
            // Set up polling since we have no real-time with API
            if (apiIntervalRef.current) clearInterval(apiIntervalRef.current);
            apiIntervalRef.current = setInterval(() => fetchViaApi(), 10000);
          }
        }
      } else {
        if (!canceled) {
          setMode('error_other');
          setErrorDetail(err.message || 'Unbekannter Fehler');
        }
      }
    };

    const qSent = query(collection(db, 'messages'), where('senderId', '==', user.id), limit(300));
    const qReceived = query(collection(db, 'messages'), where('receiverId', '==', user.id), limit(300));
    ...
    return () => {
      ...
      unsubSent?.();
      unsubReceived?.();
      if (apiIntervalRef.current) { clearInterval(apiIntervalRef.current); apiIntervalRef.current = null; }
    };
  }, [user?.id, buildConversations, fetchViaApi]);

  // IMPORTANT: activeConvo declared before scroll useEffect
  ...
  const handleSelectConvo = (partnerId: string) => {
    ...
    if (convo && user) {
      ...
      unread.forEach(m => markReadViaApi(m.id));
    }
  };
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    if (attachedFile) {
      try {
        ...
        console.error('[Postfach] Upload fehlgeschlagen:', err);
      }
    ...
    try {
      ...
      // If using API mode, refresh after send
      if (mode === 'api') {
        setTimeout(() => fetchViaApi(), 1500);
      }
    } catch (err) {
      console.error('[Postfach] Senden fehlgeschlagen:', err);
    } finally {
      setUploading(false);
    }
  };
  ...
  if (mode === 'loading') {
    return (
      <div className="h-[calc(100vh-4rem)] flex flex-col items-center justify-center bg-slate-50 gap-4">
        ...
        <p className="text-slate-400 font-bold text-sm uppercase tracking-widest">Postfach laden...</p>
      </div>
    );
  }
  ...
  if (mode === 'error_setup') {
    return (
      <div className="h-[calc(100vh-4rem)] overflow-y-auto bg-slate-50 py-10 px-4">
        <div className="max-w-xl mx-auto space-y-6">
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-6">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h2 className="font-black text-slate-900 text-lg mb-1">Einmalige Einrichtung erforderlich</h2>
                <p className="text-slate-600 text-sm font-medium">Das Postfach braucht eine einmalige Konfiguration. Wählen Sie eine der beiden Optionen:</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6">
            <div className="flex items-center gap-3 mb-4">
              <span className="w-7 h-7 bg-indigo-600 text-white rounded-full flex items-center justify-center text-xs font-black shrink-0">A</span>
              <h3 className="font-black text-slate-900">Firebase-Regeln aktualisieren (empfohlen, 2 Min.)</h3>
            </div>
            <ol className="space-y-2 text-sm text-slate-700 font-medium">
              <li className="flex gap-2"><span className="text-indigo-600 font-black shrink-0">1.</span> Öffnen Sie <span className="bg-slate-100 px-1.5 py-0.5 rounded font-mono text-xs">console.firebase.google.com</span></li>
              <li className="flex gap-2"><span className="text-indigo-600 font-black shrink-0">2.</span> Projekt <span className="font-mono bg-slate-100 px-1.5 py-0.5 rounded text-xs">hausmatch-1</span> → <strong>Firestore Database</strong> → <strong>Regeln</strong></li>
              <li className="flex gap-2"><span className="text-indigo-600 font-black shrink-0">3.</span> Den gesamten Inhalt durch die Regeln aus <span className="font-mono bg-slate-100 px-1.5 py-0.5 rounded text-xs">firestore.rules</span> ersetzen (im Repository)</li>
              <li className="flex gap-2"><span className="text-indigo-600 font-black shrink-0">4.</span> <strong>Veröffentlichen</strong> klicken → fertig!</li>
            </ol>
            <div className="mt-4 bg-slate-900 rounded-xl p-4 text-xs font-mono text-green-400 leading-relaxed">
              <span className="text-slate-500">// Wichtigste Änderung in den Regeln:</span>{'\n'}
              <span className="text-blue-400">match</span> /messages/&#123;messageId&#125; &#123;{'\n'}
              {'  '}<span className="text-yellow-400">allow list</span>: <span className="text-blue-400">if</span> isSignedIn();{'\n'}
              {'  '}<span className="text-yellow-400">allow get</span>: <span className="text-blue-400">if</span> isSignedIn() &amp;&amp; ...{'\n'}
              &#125;
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6">
            <div className="flex items-center gap-3 mb-4">
              <span className="w-7 h-7 bg-slate-600 text-white rounded-full flex items-center justify-center text-xs font-black shrink-0">B</span>
              <h3 className="font-black text-slate-900">Firebase Admin SDK in Vercel konfigurieren (5 Min.)</h3>
            </div>
            <ol className="space-y-2 text-sm text-slate-700 font-medium">
              <li className="flex gap-2"><span className="text-slate-600 font-black shrink-0">1.</span> Firebase Console → Zahnrad → <strong>Service Accounts</strong> → <strong>Neuen privaten Schlüssel generieren</strong></li>
              <li className="flex gap-2"><span className="text-slate-600 font-black shrink-0">2.</span> Vercel Dashboard → Ihr Projekt → <strong>Settings</strong> → <strong>Environment Variables</strong></li>
              <li className="flex gap-2"><span className="text-slate-600 font-black shrink-0">3.</span> Diese 3 Variablen aus der JSON-Datei hinzufügen:</li>
            </ol>
            <div className="mt-3 space-y-2">
              {[
                ['FIREBASE_ADMIN_PROJECT_ID', 'hausmatch-1'],
                ['FIREBASE_ADMIN_CLIENT_EMAIL', 'firebase-adminsdk-...@hausmatch-1.iam...'],
                ...
                <div key={key} className="bg-slate-50 rounded-lg px-3 py-2">
                  ...
                  <p className="text-xs font-mono text-slate-600 truncate">{val}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          >
            Nach Einrichtung: Seite neu laden
          </button>
        </div>
      </div>
    );
    ...
    return (
      <div className="h-[calc(100vh-4rem)] flex flex-col items-center justify-center bg-slate-50 gap-5 px-4 text-center">
        ...
        <div>
          ...
          <p className="text-slate-500 font-medium text-sm max-w-sm">{errorDetail || 'Verbindungsfehler'}</p>
        </div>
        <div className="flex gap-3">
          <button onClick={() => window.location.reload()} className="px-5 py-2.5 bg-indigo-600 text-white rounded-xl font-black uppercase text-xs tracking-widest hover:bg-indigo-700 transition-all">
            Neu laden
          </button>
          <button onClick={() => navigate('/network')} className="px-5 py-2.5 bg-white border border-slate-200 text-slate-700 rounded-xl font-black uppercase text-xs tracking-widest hover:bg-slate-50 transition-all">
            Zum Netzwerk
          </button>
        </div>
      </div>
  ...
  // --- SIDEBAR ---
  const Sidebar = (
    <div className={`flex flex-col h-full bg-white border-r border-slate-100 ${showMobileList ? 'flex' : 'hidden md:flex'} md:w-[320px] lg:w-[380px] shrink-0`}>
      <div className="px-6 pt-6 pb-4 border-b border-slate-50">
        <h2 className="text-2xl font-black text-slate-900 tracking-tighter mb-4">Postfach</h2>
        {mode === 'api' && (
          <div className="mb-3 flex items-center gap-1.5 text-[10px] font-bold text-amber-600 uppercase tracking-widest">
            <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" /></svg>
            Kein Echtzeit-Update (API-Modus)
          </div>
        )}
        ...
        {filteredConvos.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full py-16 px-6 text-center">
            ...
            <p className="text-xs text-slate-400 font-medium mb-5">
              {conversations.length === 0 ? 'Schreiben Sie Profis im Netzwerk an.' : 'Suchbegriff anpassen.'}
            </p>
            ...
            {filteredConvos.map(convo => {
              ...
              return (
                <li key={convo.partnerId}>
                  <button onClick={() => handleSelectConvo(convo.partnerId)}
                    className={`w-full text-left px-5 py-4 transition-all ${isActive ? 'bg-indigo-50' : 'hover:bg-slate-50'}`}>
                    <div className="flex items-center gap-3">
                      <div className={`w-11 h-11 rounded-2xl flex items-center justify-center font-black text-white text-base shrink-0 ${isActive ? 'bg-indigo-600' : 'bg-gradient-to-br from-indigo-400 to-indigo-600'}`}>
                        {convo.partnerName[0]?.toUpperCase() || '?'}
                      </div>
                      <div className="min-w-0 flex-1">
                        <div className="flex items-baseline justify-between gap-2 mb-0.5">
                          <span className={`text-sm font-black truncate ${isActive ? 'text-indigo-700' : 'text-slate-900'}`}>{convo.partnerName}</span>
  ...
  // --- CHAT VIEW ---
  const ChatView = (
    <div className={`flex-1 flex flex-col bg-slate-50 min-w-0 ${showMobileList ? 'hidden md:flex' : 'flex'}`}>
      {activeConvo ? (
        <>
          <div className="px-5 py-4 bg-white border-b border-slate-100 flex items-center gap-4">
            <button onClick={() => setShowMobileList(true)} className="md:hidden -ml-1 p-2 text-slate-500 hover:text-slate-900 transition-colors">
                ...
                <React.Fragment key={msg.id || idx}>
                  ...
                  <div className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                    ...
                    <div className={`max-w-[75%] sm:max-w-[65%] ${isMe ? 'items-end' : 'items-start'} flex flex-col`}>
                      <div className={`px-4 py-3 rounded-2xl shadow-sm ${isMe ? 'bg-indigo-600 text-white rounded-br-sm' : 'bg-white text-slate-800 rounded-bl-sm border border-slate-100'}`}>
                        {msg.content && <p className="text-sm leading-relaxed whitespace-pre-wrap font-medium">{msg.content}</p>}
                        {msg.attachments?.map((att, i) => <AttachmentPreview key={i} att={att} isMe={isMe} />)}
                      </div>
                      <div className={`flex items-center gap-1 mt-1 px-1 ${isMe ? 'justify-end' : ''}`}>
                        ...
                        {isMe && (
                          <svg className={`w-3 h-3 ${msg.read ? 'text-indigo-500' : 'text-slate-300'}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3">
          ...
          <div className="px-4 pb-4 pt-2 bg-white border-t border-slate-100">
            ...
            <form onSubmit={handleSend} className="flex items-end gap-2 bg-slate-100 rounded-2xl p-1.5 border border-slate-200 focus-within:border-indigo-400 focus-within:bg-white transition-all">
              <input type="file" accept={ACCEPT_TYPES} ref={fileInputRef} className="hidden" onChange={handleFileChange} />
              <button type="button" onClick={() => fileInputRef.current?.click()} title="Datei anhängen"
              ...
              />
              <button type="submit" disabled={(!replyText.trim() && !attachedFile) || uploading}
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
          ...
          <p className="text-slate-400 font-medium text-sm max-w-xs">
            {conversations.length > 0 ? 'Klicken Sie auf eine Konversation.' : 'Noch keine Nachrichten. Kontaktieren Sie Profis im Netzwerk.'}
          </p>
          {conversations.length === 0 && (
            <button onClick={() => navigate('/network')}
